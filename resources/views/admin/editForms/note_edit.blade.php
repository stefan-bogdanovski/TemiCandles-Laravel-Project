        ...
        <div class="content mt-0">
            <div class="row justify-content-center">
                <div class="col-12 col-lg-8 col-md-9 col-sm-12 text-center">
                    <h2>Izmenite ime arome: <span class="d-block colorhtag">{{$note->name}}</span></h2>
                    <form action="{{route($entity. ".update", $note->id)}}" method="POST">
                        @CSRF
                        @method('put')
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="exampleInputName">Ime</label>
                                <input type="text" class="form-control" id="exampleInputName" name="name" aria-describedby="emailHelp" value="{{$note->name}}">
                            </div>
                        </div>
                        @if ($errors->any())
